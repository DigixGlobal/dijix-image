...
describe('DijixImage', function () {
  ...
  describe('parseInput', function () {
    it('parses base64 images', async function () {
      const { mime, width, height } = await dijixImage.getImage(testImage);
      assert.deepEqual([mime, width, height], ['image/png', 16, 16]);
    });
    it('parses image buffers', async function () {
      const { mime, width, height } = await dijixImage.getImage(fs.readFileSync(`${process.env.PWD}/test/banner.jpg`));
      assert.deepEqual([mime, width, height], ['image/jpeg', 940, 250]);
    });
    it('parses file paths', async function () {
      const { mime, width, height } = await dijixImage.getImage('./test/banner.jpg');
      assert.deepEqual([mime, width, height], ['image/jpeg', 940, 250]);
    });
  });
  ...
  describe('creationPipeline', function () {
    it('creates the dijix object data (defaults)', async function () {
      assert.deepEqual(await dijixImage.creationPipeline({ src: './test/banner.jpg' }, mockDijix), {
        height: 250,
        width: 940,
        src: 'Fake IPFS Hash',
        size: 256177,
